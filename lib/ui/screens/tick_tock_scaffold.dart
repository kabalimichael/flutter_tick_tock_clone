...
//Control the value of offsetX between -screenWidth and screenWidth
  ...
  onHorizontalDragEnd(details, screenWidth) {
    ...
    //  Animate according to the offset of offsetX when the sliding stops
...
// Calculate [offsetY]
  ///
  /// Slide your finger up, [absorbing] is false, the event is not blocked, and the event is handled by the underlying PageView
  /// It is on the first page and it is a drop-down, then the sliding piece is intercepted
  void calculateOffsetY(DragUpdateDetails details) {
    ...
    if (widget.currentIndex == 0) {
      // absorbing = true; // TODO: 
// Pull to refresh temporarily blocked
      if (tempY > 0) {
        if (tempY < 40) {
          offsetY = tempY;
        } else if (offsetY != 40) {
    ...
    return AbsorbPointer(
      ...
      child: NotificationListener<OverscrollIndicatorNotification>(
        ...
        child: NotificationListener<UserScrollNotification>(
          onNotification: (notification) {
            // When the finger leaves and is at the top, the PageView sliding event is intercepted. TODO: The pull-down refresh is not triggered
...
/// Widget on the left
///
/// Use [Transform.scale] to scale according to [offsetX]
/// The minimum is 0.88 and the maximum is 1
